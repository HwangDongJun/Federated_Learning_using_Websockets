	...
	async def _configuration_handler(self, websocket: websockets.WebSocketCommonProtocol):
		for i in range(self.first_conn_count):
			try:
				...
				# (differential privacy)config : Information needed to train the model to send to the clinets.
				# image_shape, batch_size, learning_rate, epochs, optimizer_method, model_type, l2_norm_clip, noise_multiplier, num_microbatches, training_data_size, delta
				config = '96,32,0.002,1,dp,mobilenetv2,0.5,0.3,32,60000,1e-4'
